...
public class NightEdgeBladeAttackHandler {
    private static final int ATTACK_INTERVAL = 6; // ✅ 攻撃間隔 (0.5秒 = 10 ticks)
    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        if (!(weapon.getItem() instanceof NightEdge)) {
    ...
    private static void createAreaAttack(Player player, Level level) {
        ...
        for (Entity entity : entities) {
            if (entity instanceof LivingEntity && entity != player) {
                ...
                // ✅ 無敵時間をリセットして連続ダメージを適用
                target.invulnerableTime = 0;

                // ✅ ダメージを適用
                target.hurt(DamagetypeRegister.causeNotchDamage(player), 20);
            }
        }
    }
}
